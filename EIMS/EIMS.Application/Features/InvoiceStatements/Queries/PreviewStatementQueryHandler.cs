...
{
    internal class PreviewStatementQueryHandler: IRequestHandler<PreviewStatementQuery, Result<string>>
    {
        ...
        public PreviewStatementQueryHandler(IUnitOfWork uow, IStatementService statementService, IPdfService pdfService, IInvoiceXMLService invoiceXMLService)
        {
            _uow = uow;
        ...
        public async Task<Result<string>> Handle(PreviewStatementQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                return Result.Fail($"Lỗi convert HTML: {ex.Message}");
            }
        }
    }
}
